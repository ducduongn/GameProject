            ...
            if (brick.state) {
                ...
                if (ball->x <= brickx + BOARD_BRWIDTH && ball->x+ball->width >= brickx
                    ...
                    if ( board -> bricks[i][j].status == 0 ){
                        ...
                        /* desRect.x = i*( BOARD_BRWIDTH/2 - EXPLODE_WIDTH/2 );
                        desRect.y = j*( BOARD_BRHEIGHT/2 - EXPLODE_HEIGHT/2 );
                        desRect.w = EXPLODE_WIDTH;
                        desRect.h = EXPLODE_HEIGHT;
                        SDL_RenderCopy(renderer,texture, &frame_clip[explosion_frame], &desRect);
                        SDL_RenderPresent(renderer);

                        explosion_frame++;
                        if (explosion_frame == 48) explosion_frame = 0;
                     */
...
void Game :: Sound() {
    Mix_Chunk *wave = NULL;
    wave = Mix_LoadWAV("Sound//hit.wav");
	if (wave == NULL)
